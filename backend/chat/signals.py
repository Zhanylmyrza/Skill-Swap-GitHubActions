...
def create_user_profile(sender, instance, created, **kwargs):
    if created:
        profile = UserProfileModel.objects.create(user=instance, name=instance.username)
        profile.save()
...
def send_notification(sender, instance, created, **kwargs):
    if created:
        ...
        async_to_sync(channel_layer.group_send)(
            user_id, {"type": "send_notification", "value": json.dumps(data)}
        )
...
def send_onlineStatus(sender, instance, created, **kwargs):
    if not created:
        ...
        data = {"username": user, "status": user_status}
        async_to_sync(channel_layer.group_send)(
